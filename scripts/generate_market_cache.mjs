...
const tickers = ["MSFT", "GOOG", "AAPL", "BAC", "NVDA"];
...
async function buildTickerData(ticker) {
  ...
  if (closes.length < 30) {
    throw new Error(`Insufficient chart data for ${ticker}.`);
  }
  ...
  let expiries = rawExpiries
    ...
    .slice(0, 8);
  if (!expiries.length) expiries = rawExpiries.slice(0, 6);
  if (!expiries.length) throw new Error(`No option expiries for ${ticker}.`);
  ...
  for (let expiryIndex = 0; expiryIndex < expiries.length; expiryIndex += 1) {
    ...
    const optionChain = expiryIndex === 0 ? rootOptions : await yahooFinance.options(ticker, { date: expiry });
...
async function main() {
  ...
  for (const ticker of tickers) {
    try {
      records[ticker] = await buildTickerData(ticker);
      console.log(`cache: ${ticker} ok`);
    } catch (error) {
      console.warn(
        `cache: ${ticker} failed`,
  ...
  const outputPath = path.resolve(process.cwd(), "public", "market-data-cache.json");
  await fs.mkdir(path.dirname(outputPath), { recursive: true });
  await fs.writeFile(outputPath, JSON.stringify(payload), "utf8");
  console.log(`cache: wrote ${outputPath}`);
}
